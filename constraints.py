...
def create_fit_limit_constraint(max_v, min_v, variables):
	def fit_limit_constraint(vars_f: Dict, bags_f: Dict, curr: Dict) -> bool:
		...
		if len(all_items) != len(vars_f):
			return all(map(lambda bag: len(bag) <= max_v, curr.items()))
...
def create_unary_inclusive_constraint(vals):
	...
	def unary_inclusive_constraint(vars_f: Dict, bags_f: Dict, curr: Dict) -> bool:
		contains_item = {bag_name: item in contents for bag_name, contents in curr.items()}
...
def create_unary_exclusive_constraint(vals):
	def unary_exclusive_constraint(vars_f: Dict, bags_f: Dict, curr: Dict) -> bool:
		return not any(k in curr.keys() and vals[0] in curr[k] for k in vals[1:])
...
def create_binary_equals_constraint(vals):
	...
	def binary_equals_constraint(vars_f: Dict, bags_f: Dict, curr: Dict) -> bool:
		...
		return len(list(filter(lambda bag: item2 in bag or item1 in bag, curr.values()))) == 1
...
def create_binary_not_equals_constraint(vals):
	...
	def binary_not_equals_constraint(vars_f: Dict, bags_f: Dict, curr: Dict) -> bool:
		...
		return len(list(filter(lambda bag: item2 in bag or item1 in bag, curr.values()))) == 2
